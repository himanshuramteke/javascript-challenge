...
function findMedianSortedArrays(nums1, nums2) {
    ...
    while (left <= right) {
        ...
        let maxLeft1 = partition1 === 0 ? Number.NEGATIVE_INFINITY : nums1[partition1 - 1];
        let minRight1 = partition1 === len1 ? Number.POSITIVE_INFINITY : nums1[partition1];
        
        let maxLeft2 = partition2 === 0 ? Number.NEGATIVE_INFINITY : nums2[partition2 - 1];
        let minRight2 = partition2 === len2 ? Number.POSITIVE_INFINITY : nums2[partition2];
...
testCases.forEach((test, index) => {
    console.log(`Test Case ${index + 1}: Median is ${findMedianSortedArrays(test.nums1, test.nums2)}`);
});
